...
subString start end str
    | end < 0       = ""
    | start > end   = ""
    | otherwise     = take (adjustedEnd - adjustedStart + 1) (drop adjustedStart str)
    where
        adjustedStart = max 0 start
        adjustedEnd = min end (length str - 1)
...
-}
strSum :: String -> Int
strSum str = sum ( map (\x -> read x) (words str) )
...
lowerAndGreater n list = go 0 0 list
    where
        ...
        go greater lower l =
            if null l
            then (show n) ++ " is greater than " ++ (show lower) ++ " elements and lower than " ++ (show greater) ++ " elements"
            else if head l > n
                then go (greater + 1) (lower) (tail l)
                else if head l < n
                    then go (greater) (lower + 1) (tail l)
                    else go (greater) (lower) (tail l)
